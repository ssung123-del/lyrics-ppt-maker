...
가사와 배경 이미지를 조합해 `.pptx` 파일을 만들고, 가사 기반 배경 이미지 프롬프트까지 생성하는 React 기반 웹 앱입니다.
...
- 가사에서 핵심 키워드와 무드를 뽑아, 가독성을 우선한 한국어 배경 이미지 프롬프트를 만듭니다.
...
3. 자동 생성된 `배경 이미지 프롬프트`를 복사합니다.
4. 외부 이미지 생성 도구에서 `중앙 여백`, `낮은 디테일`, `추상적 분위기`를 유지한 배경을 만듭니다.
5. 생성한 배경 이미지를 업로드합니다.
6. 글자색, 그림자, 폰트 크기를 조절합니다.
7. 오른쪽 미리보기에서 슬라이드별 결과를 확인합니다.
8. `PPT 다운로드`를 눌러 파일을 생성합니다.
...
src/
  ...
  utils/
    backgroundPrompt.ts     가사 -> 이미지 생성 프롬프트 변환
    backgroundPrompt.test.ts 프롬프트 생성 로직 테스트
...
[`src/utils/backgroundPrompt.ts`](./src/utils/backgroundPrompt.ts)

- 가사에서 시각적으로 의미 있는 단어를 우선 추출합니다.
- 예배 분위기를 `평안`, `새벽`, `승리`, `기도`, `회복` 계열로 분류합니다.
- 배경은 `중앙 여백`, `낮은 디테일`, `가장자리 위주 디테일`, `상징적 표현`을 기본 원칙으로 잡습니다.
- 한국어 프롬프트와 배경 방향 가이드를 함께 제공합니다.
- 이 과정은 외부 AI 호출 없이 규칙 기반으로만 동작합니다.
...
- 프롬프트 생성은 규칙 기반이므로 문맥 해석이 사람 수준으로 깊지는 않습니다.
- 이미지 생성 모델마다 프롬프트 해석 차이가 있어, 너무 디테일한 결과가 나오면 프롬프트에 `중앙 여백`, `낮은 디테일`, `추상적` 조건을 더 강하게 넣는 편이 좋습니다.
...
- 예배용 가사:
  먼저 자동 생성 프롬프트를 복사한 뒤 이미지 생성기에서 배경을 만들고 업로드하는 흐름이 가장 안정적입니다.
- 자막 중심 발표:
